resource "alicloud_instance" "monitor" {
    count = var.instance_count["monitor"]
    instance_name         = "terraform_monitor"
...
output "monitorSerPublicIP" {
  depends_on = [ alicloud_instance.monitor ]
  value = join("\n", [
                    "[monitorSer]",
                    join("\n", [for idx, monitor in alicloud_instance.monitor:  
                                "${idx}-${monitor.public_ip}-${monitor.private_ip}"
                                ])
                    ])
  description = "节点公网IP地址"
}

resource "null_resource" "monitorSerENVInit" {
  depends_on = [ alicloud_instance.monitor ]
  provisioner "local-exec" {
    command = <<EOT
cat << EOF >> ${path.module}/../ansible-playbooks/inventory/allNodes.ini
${join("\n", [for monitor in alicloud_instance.monitor: monitor.public_ip])}
EOF
...
cat << EOF >> ${path.module}/../ansible-playbooks/inventory/biz_monitor.ini
${join("\n", [for monitor in alicloud_instance.monitor: monitor.public_ip])}
EOF
...
resource "null_resource" "monitorSerSSHConfig" {
  depends_on = [ alicloud_instance.monitor ]
  provisioner "local-exec" {
    command = <<EOT
    ...
    join("\n", [for idx, monitor in alicloud_instance.monitor: 
      join("\n", [
        format("Host monitorSer-%s", idx),
        format("Hostname %s", monitor.public_ip),
